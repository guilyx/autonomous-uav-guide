...
class TestOccupancyGrid:
    def test_shape_2d(self):
        g = OccupancyGrid(resolution=1.0, bounds_min=np.zeros(3), bounds_max=np.full(3, 5.0))
    ...
    def test_set_and_query(self):
        g = OccupancyGrid(resolution=0.5, bounds_min=np.zeros(3), bounds_max=np.full(3, 5.0))
    ...
    def test_from_world(self):
        ...
        g = OccupancyGrid(resolution=1.0, bounds_min=np.zeros(3), bounds_max=np.full(3, 10.0))
        g.from_world(w)
...
class TestSocialLayer:
    def test_stationary_agent_no_cost(self):
        g = OccupancyGrid(resolution=0.5, bounds_min=np.zeros(3), bounds_max=np.full(3, 10.0))
        w = World()
        w.add_agent(DynamicAgent(position=np.array([5.0, 5.0, 0.0]), velocity=np.zeros(3)))
        layer = SocialLayer()
    ...
    def test_moving_agent_creates_cost(self):
        ...
        w = World()
        w.add_agent(
            DynamicAgent(position=np.array([5.0, 5.0, 0.0]), velocity=np.array([2.0, 0.0, 0.0]))
        )
        layer = SocialLayer()
        cost = layer.apply(g, w)
        assert np.max(cost) > 0.0
