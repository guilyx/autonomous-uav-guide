...
from flybots.costmap import LayeredCostmap
from flybots.environment import default_world
from flybots.logging import SimLogger
from flybots.path_tracking.flight_ops import fly_mission
...
def main() -> None:
    ...
    logger = SimLogger("astar_3d", out_dir=Path(__file__).parent)
    logger.log_metadata("algorithm", "A*")
    logger.log_metadata("nodes_explored", len(explored))
