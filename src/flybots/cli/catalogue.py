...
Simulations live at ``flybots/simulations/<category>/<name>/run.py``. Rather
...
class SimulationEntry:
    ...
    @property
    def module(self) -> str:
        return f"flybots.simulations.{self.category}.{self.name}.run"

    @property
    def command(self) -> str:
        return f"python -m flybots.simulations.{self.category}.{self.name}"
    ...
    def load(self):
        ...
        # through discover().
        if not module_name.startswith("flybots.simulations."):
            raise ValueError(f"refusing to import {module_name!r}: outside flybots.simulations")
