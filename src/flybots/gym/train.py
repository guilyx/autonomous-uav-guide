...
The optimiser is pluggable; see :mod:`flybots.gym.optimizers`. The default
is Augmented Random Search.

    >>> from flybots.gym.train import train
...
from flybots.gym.optimizers import build_optimizer
from flybots.gym.policy import MLPPolicy
from flybots.gym.registry import make
...
class TrainConfig:
    ...
    optimizer: str = "ars"
    """``"ars"`` (default) or ``"cem"``. See :mod:`flybots.gym.optimizers`."""
    iterations: int = 120
    directions: int = 16
...
def train(
    ...
    *,
    ...
    ----------
    env_id
        Environment name, see :func:`flybots.gym.registry.list_envs`.
